...
const addProductId = async (
  ...
  try {
    const productRequest = await prisma.productRequest.findUnique({
      where: {
        id: productRequestPayload?.productRequestId,
      },
    });
  ...
  try {
    ...
    if (productRequestQAPayload.form.length === 1) {
      const data = {
        ...
        productRequestId: findproductRequest.id,
      ...
      if (productRequestQAPayload.form[0].subForm) {
        const productRequestForm = productRequestQAPayload.form[0].profile.map(
          (data: ProfileData) => ({
            ...
            productRequestQAId: createproductRequestFormData.id,
          })
        );
    ...
    logger.info({
      message: `Form saved successfully for Product Request with ${productRequestId} `,
    });
